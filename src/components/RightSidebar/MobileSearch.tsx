// MobileSearch.tsx - Fixed version
import React, { useEffect, useRef, FormEvent, KeyboardEvent, MouseEvent } from 'react';
...
const MobileSearchSuggestions: React.FC<MobileSearchSuggestionsProps> = ({ 
  suggestions, 
  searchTerm, 
  onSuggestionClick 
}) => {
...
interface MobileSearchProps {
  ...
  onSearch: (e: FormEvent<HTMLFormElement> | MouseEvent<HTMLButtonElement>) => void;
  suggestions: string[];
  onSuggestionClick: (suggestion: string) => void;
}
...
const MobileSearch: React.FC<MobileSearchProps> = ({ 
  ...
  const handleKeyDown = (e: KeyboardEvent<HTMLInputElement>): void => {
    if (e.key === 'Enter') {
      ...
      onSearch(e as any); // Type assertion needed for compatibility
      handleClose();
    }
    if (e.key === 'Escape') {
  ...
  const handleSearchSubmit = (e: MouseEvent<HTMLButtonElement>): void => {
    e.preventDefault();
    if (searchTerm.trim()) {
      onSearch(e);
      handleClose();
